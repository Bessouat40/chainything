...
impl LeftPanel {
    ...
    fn add_node(&self, node_name: &String, node_registry: &NodeRegistry, snarl: &mut Snarl<Box<dyn BaseNode>>, ctx: &egui::Context) {
        node_registry.create_node(node_name).and_then(|node| {
    ...
    pub fn show(&mut self, ui: &mut egui::Ui, snarl: &mut Snarl<Box<dyn BaseNode>>, node_registry: &NodeRegistry) {
        egui::Panel::left("left_panel")
            ...
            .show_inside(ui, |ui| {
                ...
                egui::ScrollArea::vertical().show(ui, |ui| {
                    for node in node_registry.nodes.keys() {
                        if !search_lower.is_empty() && !node.to_lowercase().contains(&search_lower) {
                            continue;
                        }
