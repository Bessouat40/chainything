...
#[derive(Clone)]
pub struct ModelReaderNode {
    path_input: RefCell<String>,
}

impl ModelReaderNode {
    pub fn new() -> Self {
        Self {
...
impl BaseNode for ModelReaderNode {
    fn name(&self) -> &str {
        "ModelReader"
    }
    ...
    fn informations(&self) -> NodeInformations {
        NodeInformations::new(
            "Reads a 3D model (Wavefront OBJ) from the configured file path and outputs it as a mesh.",
        )
    }
