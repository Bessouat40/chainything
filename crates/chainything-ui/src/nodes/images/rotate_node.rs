...
impl BaseNode for RotateNode {
    ...
    fn informations(&self) -> NodeInformations {
        NodeInformations::new(
            "Rotates an image clockwise by 90, 180 or 270 degrees. For 90 and 270 the width and height are swapped.",
        )
    }
    ...
    fn show_body(
        ...
        ui.horizontal(|ui| {
            ui.label("Angle:");
            let mut selected = self.angle.get();
            egui::ComboBox::from_id_salt(("rotate_angle", node))
                .selected_text(format!("{}°", selected))
                .show_ui(ui, |ui| {
                    ui.selectable_value(&mut selected, 90, "90°");
                    ui.selectable_value(&mut selected, 180, "180°");
                    ui.selectable_value(&mut selected, 270, "270°");
                });
            self.angle.set(selected);
        });
    }
