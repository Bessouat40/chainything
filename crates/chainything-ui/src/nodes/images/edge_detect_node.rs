...
impl BaseNode for EdgeDetectNode {
    fn name(&self) -> &str {
        "EdgeDetect"
    }
    ...
    fn informations(&self) -> NodeInformations {
        NodeInformations::new(
            "Detects edges using the Sobel operator. The image is converted to greyscale, then the gradient magnitude is returned so bright pixels mark strong edges.",
        )
    }
